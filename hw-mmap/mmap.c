...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
  ...
  if(last_addr == NULL)
  {
    last_addr = align_down(fault_addr, page_size);
  }
  else
  {
    if (munmap((void*)last_addr, page_size) == -1) {
    fprintf(stderr, "Couldn't munmap() region for sqrt table page; %s\n",
            strerror(errno));
    }
    last_addr = align_down(fault_addr, page_size);
  }
  addr = mmap(last_addr, page_size, PROT_READ|PROT_WRITE,
	       MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (addr == MAP_FAILED) {
    fprintf(stderr, "Couldn't mmap() region for sqrt table page; %s\n",
	    strerror(errno));
    exit(EXIT_FAILURE);
  }
  calculate_sqrts(addr, addr - sqrts, page_size/sizeof(double));
